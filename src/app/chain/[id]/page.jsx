...
const Page = () => {
  ...
  const [pageTitle, setPageTitle] = useState(
    `Story Chain ${id} - Quilted Chronicles`
  );
  const [pageDescription, setPageDescription] = useState(
    `Explore this story chain on Quilted Chronicles. Follow different narrative branches and discover how multiple authors have continued this collaborative tale.`
  );
  ...
  useEffect(() => {
    ...
    updateMetaTag("og:url", `https://www.quiltedchronicles.com/chain/${id}`);
    ...
    updateMetaTag(
      "keywords",
      "story chain, narrative paths, collaborative story, multiple endings, story branches"
    );
  ...
  useEffect(() => {
    ...
    updateMetaTag("canonical", `https://www.quiltedchronicles.com/chain/${id}`);
  ...
  const handleTitleChange = ({
    ...
    // Set story title, author and publication date for meta tags
    setStoryTitle(title);
    setAuthorName(author);
    setPublicationDate(createDate);

    if (title) {
      setPageTitle(`${validator.unescape(title)} | Quilted Chronicles`);
      setPageDescription(
        `Explore "${validator.unescape(
          title
        )}" by ${author}. Follow different narrative paths and discover how multiple authors have continued this collaborative tale.`
      );
    }
  };
  ...
  return (
    <div>
      <Chain onTitleChange={handleTitleChange} />
    </div>
  );
};
